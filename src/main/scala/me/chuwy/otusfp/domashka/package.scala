...
package object domashka {

  case class CounterDTO(counter: Long)

  case class SlowParams(chunk: Int, total: Int, time: Int)

  case class SlowParamsToCheck(chunk: String, total: String, time: String) {
    ...
    def validate: ValidatedNec[String, SlowParams]  = (
      validateInt(this.chunk, "chunk is not integer greater than 0").toValidatedNec,
      validateInt(this.total, "total is not integer greater than 0").toValidatedNec,
      validateInt(this.time, "time is not integer greater than 0").toValidatedNec,
